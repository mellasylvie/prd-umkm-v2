...
import { BarChart as RechartsBarChart, LineChart, XAxis, YAxis, Bar, Line, CartesianGrid, Tooltip, Legend } from "recharts"
import { BarChart, Users, Package, DollarSign, Activity, MousePointerClick } from "lucide-react"
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="text-muted-foreground">Analisis performa minisite Anda.</p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Tren kunjungan minisite Anda selama 30 hari terakhir.</CardDescription>
        </CardHeader>
        <CardContent>
          <ChartContainer config={chartConfig} className="h-[300px] w-full">
            <LineChart data={chartData.map((d, i) => ({ ...d, day: i + 1, clicks: Math.floor(d.visits / (Math.random() * 5 + 2)) }))} margin={{ left: 12, right: 12 }}>
              <CartesianGrid vertical={false} />
              <XAxis dataKey="day" tickLine={false} axisLine={false} tickMargin={8} tickFormatter={(value) => `Hari ${value}`} />
              <YAxis />
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          ...
          <CardContent>
            <ChartContainer config={{ visits: { label: "Kunjungan", color: "hsl(var(--primary))" } }} className="h-[300px] w-full">
              <RechartsBarChart data={barChartData} layout="vertical" margin={{ left: 20 }}>
